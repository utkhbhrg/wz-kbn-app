      ...
      for (let idx in this.apiEntries) {
        try {
          ...
          const downReason = typeof error === 'string' ? error :
            (error || {}).message || ((error || {}).data || {}).message || 'EH-ISMAC is not reachable';
      ...
      return [
        {
          date: new Date(),
          level: 'error',
          message: 'Error when loading EH-ISMAC logs'
        }
      ];
    }
  }
    ...
    try {
      ...
      if (notRecheable) {
        if (notRecheable >= hosts.length) {
          ...
          throw {
            message:
              'EH-ISMAC API not recheable, please review your configuration',
            type: 'danger',
            closedEnabled: true
          };
        }
        throw {
          message: `Some of the API entries are not reachable. You can still use the EH-ISMAC APP but please, review your hosts configuration.`,
          type: 'warning',
          closedEnabled: true
        };
      }
