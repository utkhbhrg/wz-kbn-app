        ...
        if (this.metricsList[section]) {
            return this.metricsList[section].map((item, idx) => {
                ...
                return (react_1.default.createElement(eui_1.EuiFlexItem, { grow: count > 20 ? 3 : 1, key: `${item.name}` },
                    react_1.default.createElement(eui_1.EuiStat, { title: react_1.default.createElement("span", { style: { fontSize: count > 20 ? "2rem" : "2.25rem" } }, this.state.results[item.name]), description: item.name, titleColor: this.metricsList[section][idx].color || 'primary', isLoading: this.state.loading, textAlign: "center" })));
            });
        }
    }
    render() {
