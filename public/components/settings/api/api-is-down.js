...
export class ApiIsDown extends Component {
  ...
  render() {
    const apiExample = `# Example EH-ISMAC API configuration
hosts:
    - production:
        url: http://172.16.1.2
        port: 55000
...
`;
    ...
    const checkConnectionChildren = (
      <div>
        <EuiText>
          Check that the Kibana server can reach the configured EH-ISMAC API(s).
        </EuiText>
        <EuiSpacer />
        <EuiButton
        ...
        <EuiSpacer />
        <EuiText>Already configured EH-ISMAC API(s)</EuiText>
        <EuiSpacer />
        {(!this.state.error && (
          <EuiBasicTable
    ...
    const steps = [
      {
        title: 'Check the EH-ISMAC API service status',
        children: (
          <div>
