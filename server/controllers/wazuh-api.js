...
export class WazuhApiCtrl {
  ...
  async checkStoredAPI(req, reply) {
    try {
      ...
      if (!Object.keys(api).length) {
        throw new Error('Could not find EH-ISMAC API entry on wazuh.yml');
      }
      ...
      if (this.checkResponseIsDown(response)) {
        return ErrorResponse(
          `ERROR3099 - ${response.body.message || 'EH-ISMAC not ready yet'}`,
          3099,
          500,
          reply
        );
  ...
  async checkAPI(req, reply) {
    try {
      ...
      if (parseInt(response.statusCode) === 401) {
        log('wazuh-api:checkAPI', `Wrong EH-ISMAC API credentials used`);
      ...
      const tmpMsg =
        ...
        'Unexpected error checking the EH-ISMAC API';
      ...
      if (error.code === 'EPROTO') {
        return ErrorResponse(
          'Wrong protocol being used to connect to the EH-ISMAC API',
          3005,
          500,
          reply
        );
  ...
  async checkDaemons(api, path) {
    try {
      ...
      if (!isValid) {
        throw new Error('EH-ISMAC not ready yet');
      }
    } catch (error) {
  ...
  async makeRequest(method, path, data, id, reply) {
    ...
    try {
      ...
      if (path === '/ping') {
        try {
          ...
          if (!isDown) {
            log(
              'wazuh-api:makeRequest',
              'EH-ISMAC API is online but ISMAC is not ready yet'
            );
            return ErrorResponse(
              `ERROR3099 - ${response.body.message || 'EH-ISMAC not ready yet'}`,
              3099,
      ...
      throw responseError && responseBody.message
        ...
        : new Error('Unexpected error fetching data from the EH-ISMAC API');
    } catch (error) {
      ...
      if (devTools) {
        ...
        return ErrorResponse(
          error.message || error,
          error.code ? `EH-ISMAC API error: ${error.code}` : 3013,
          500,
          reply
        );
      }
  ...
  getTimeStamp(req, reply) {
    try {
      ...
      if (source.installationDate && source.lastRestart) {
        ...
        throw new Error('Could not fetch EH-ISMAC-version registry');
      }
    } catch (error) {
      log('wazuh-api:getTimeStamp', error.message || error);
      return ErrorResponse(
        error.message || 'Could not fetch EH-ISMAC-version registry',
        4001,
        500,
        reply
      );
  ...
  async getSetupInfo(req, reply) {
    try {
      ...
      return ErrorResponse(
        `Could not get data from EH-ISMAC-version registry due to ${error.message ||
          error}`,
        4005,
        500,
        reply
